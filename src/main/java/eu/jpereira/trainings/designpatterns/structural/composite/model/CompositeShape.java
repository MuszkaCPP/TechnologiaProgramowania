...
public abstract class CompositeShape extends Shape {
	...
	public void move(int xIncrement, int yIncrement) {
		...
		for(Shape _shape : getShapes()) {
			_shape.move(xIncrement, yIncrement);
		}
	}
	...
	public List<Shape> getShapesByType(ShapeType circle) {
		List<Shape> tempArray = new ArrayList<Shape>();
	...
	public List<Shape> getLeafShapes() {
		List<Shape> tempArray = new ArrayList<Shape>();
		for(Shape shape: getShapes()){
			if(shape.getClass().getSuperclass() == LeafShape.class) {
				tempArray.add(shape);
			}
		}
		return tempArray;
	 ...
	 * @return
	 */
	protected ArrayList<Shape> createShapesList() {
